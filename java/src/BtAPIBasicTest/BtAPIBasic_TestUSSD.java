 ...
 * @author Joao Paredes
 */
public class BtAPIBasic_TestUSSD {
// btTelco API URL     
...
// Set the message properties    
    private static final String USSD_TO = ""; // Destination number. Do not include any prefix such as 00 nor plus (+) sign
    private static final String USSD_BODY = "Hello. This is a test message from btTelco. Have a nice day.";
    // private static final String  SMS_FROM  =  "YourBrand";
    //private static final String  SMS_FROM  = "YourNumber";
    private static final String USSD_FROM = "false";        // When set to false, your account default number will be used as Sender ID

    public static void main(String[] args) throws InterruptedException {
        BtAPIBasic bttelco = new BtAPIBasic(BTTELCO_API_URL, ACCT_NAME, ACCT_KEY);
        HashMap send_data = bttelco.ussd.send(USSD_TO, USSD_BODY, USSD_FROM, null);      
        // send_data = bttelco.ussd.send(USSD_TO, USSD_BODY) # This is also valid. Default values will be used.

        // Check for errors
        if (send_data == null) {
            // Something went wrong
            System.out.println("Unable to send USSD.");
            return;
        }
        ...
        HashMap report_data = bttelco.ussd.report(new Integer(mId));
